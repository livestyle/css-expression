...
define(function(require, exports, module) {
	...
	function getDelta(expr1, expr2, context) {
		var sign = '+';
		var delta = evaluator('(' + v2 + ') - (' + v1 + ')', context);
		...
		if (isZero(delta)) {
			if (delta.is(tok.color)) {
				...
				var delta2 = evaluator('(' + v1 + ') - (' + v2 + ')', context);
				if (!isZero(delta2)) {
					delta = delta2;
					sign = '-';
				}
	...
	return {
		 ...
		 * cannot be patched
		 */
		replaceSafeToken: function(expr, value, safe) {
			var tokens;
		...
		patch: function(expr, context, expected, actual) {
			if (!Array.isArray(expr)) {
			...
			var delta;
			try {
				delta = getDelta(actual, expected, context);
			} catch (e) {
			...
			if (safe) {
				...
				return this.replaceSafeToken(expr, safeValue, safe);
			} else {
				// no safe token: simply add delta
				return expr.expression + ' ' + delta.sign + ' ' + delta.valueOf();
			}
		}
	};
});